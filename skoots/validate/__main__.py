...
from skoots.validate.lib import get_segmentation_errors, mask_iou, accuracies_from_iou, f1_score, _iou_instance_dict, mask_dice, mask_soft_cldice
...
def main() -> None:
    ...
    parser.add_argument('--ground_truth', type=str, help='Path to ground truth instance mask')
    parser.add_argument('--predicted', type=str, help='Path to ground truth instance mask')
    ...
    with open(f'{file_without_extension}_accuracy_stats.csv', 'w') as file:
        ...
        file.write(f'thr,true_positive,false_positive,false_negative,precision,recall,f1\n')
        i = -1
    ...
    with open(f'{file_without_extension}_intersection_over_union.csv', 'w') as file:
        ...
        for i, u in enumerate(gt.unique()):
            if u == 0: continue
            file.write(f'{u},{iou[i-1, :].max().item()},{dice[i-1, :].max().item()},{cldice[i-1, :].max().item()}\n')
