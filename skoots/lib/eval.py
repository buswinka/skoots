...
from skoots.lib.morphology import binary_erosion, binary_dilation, binary_dilation_2d
...
def eval(image_path: str,
    ...
    iterator = tqdm(crops(image, cropsize, overlap, device=device), desc='', total=total)
    ...
    for crop, (x, y, z) in iterator:
        ...
        for _ in range(1): # expand the skeletons in x/y/z. Only  because they can get too skinny
    ...
    logging.info(f'Saving intermediate outputs')
    ...
    cropsize = [500, 500, 50]
    overlap = (50, 50, 5)
    iterator = tqdm(crops(vectors, crop_size=cropsize, overlap=overlap), desc='Assigning Instances:')

    logging.info(f'Identifying connected components...')
    for _vec, (x, y, z) in iterator:

        _destination = (slice(x + overlap[0], x + cropsize[0] - overlap[0]),
                        slice(y + overlap[1], y + cropsize[1] - overlap[1]),
                        slice(z + overlap[2], z + cropsize[2] - overlap[2]),
                        )

        _source = (slice(overlap[0], -overlap[0]),
                   slice(overlap[1], -overlap[1]),
                   slice(overlap[2], -overlap[2]))

        _embed = skoots.lib.vector_to_embedding.vector_to_embedding(scale=vector_scale, vector=_vec, N=2)
        ...
        instance_mask[_destination] = _inst_maks[_source] if torch.tensor(overlap).gt(0).all() else _inst_maks
    ...
    instance_mask, remapping = fastremap.renumber(instance_mask.cpu().numpy(), in_place=True)

    # io.imsave(filename_without_extensions + '_instance_mask.tif',
    #           instance_mask.transpose(2, 0, 1), compression='zlib')
    zarr.save_array(filename_without_extensions + f'_skoots_instance_mask.zarr', instance_mask)
