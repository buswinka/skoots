...
def crops(image: Tensor,
          crop_size: List[int],
          overlap: Optional[Tuple[int]] = (0, 0, 0),
          device='cpu') -> Tuple[Tensor, List[int]]:
    """
    ...
    for c, o, d in zip(crop_size, overlap, dim):
        assert c - (
                    o * 2) != 0, f'Overlap in {d} dimmension cannot be equal to or larger than crop size... {c=} - {o*2=} = {c - (o * 2)} < {c}'
        ...
        while y < image_shape[2]:
            ...
            while z < image_shape[3]:
                ...
                _output_cache.append((
                    ...
                    _x:_x + crop_size[0],
                    _y:_y + crop_size[1],
                    _z:_z + crop_size[2]].unsqueeze(0).to(device, non_blocking=True), [_x, _y, _z]
                ))
